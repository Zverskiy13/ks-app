...
const API = {
  ...
  healthFileGet(id) { return USE_REMOTE ? fetch(`${API_BASE}/health/file?id=${encodeURIComponent(id)}`).then(r => r.json()) : Promise.resolve({ ok: false }); },
  ...
  assistant(profile, mode, question, health) { return this._post("assistant", { user: profile.id, mode: mode || "cached", question: question || "", health: health || {} }); },
